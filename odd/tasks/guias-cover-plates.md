...
- [ ] Generate cover 1 and verify it; then the other two.
- [ ] Trim each accepted cover to 16:9 and look at the result before integrating.
- [ ] Integrate: `cover.tsx` renders `IllustrationPlate` at the settled frame, with
      `object-cover` and `priority` kept, and the three `portada` paths resolve to the
      new files. The third guide's `portada` changes extension from `.jpg` to `.png`,
      and all three `portadaAlt` values are rewritten, because two of them still say
      "mostrada como un relieve de teselas".
- [ ] Verify with the user in the browser, both themes, in `pnpm dev`.
- [ ] Verify: typecheck, scoped lint, full test suite, every referenced
      `/img/guias/` path present on disk.
- [ ] Commit as work units on this branch and record the identities here.
