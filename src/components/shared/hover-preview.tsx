...
const HoverPreview = ({
	...
	const handleMouseMove = useCallback(
		(index: number, event: React.MouseEvent<HTMLSpanElement>) => {
			...
			const offX = Math.max(-maxOffset, Math.min(maxOffset, (deltaX / rect.width) * maxOffset * 2))
			const offY = Math.max(-maxOffset, Math.min(maxOffset, (deltaY / rect.height) * maxOffset * 2))

			rotation.set(rot)
			offsetX.set(offX)
			offsetY.set(offY)
		},
		[hoveredIndex, maxRotation, maxOffset, cursorX, cursorY, rotation, offsetX, offsetY]
	)
			...
			if (targets[placeholderIndex]) {
				...
				parts.push(
					<span
						...
						className={cn("relative cursor-pointer transition-colors", targetClassName)}
						style={{
