...
const NeonReveal: React.FC<NeonRevealProps> = ({
	...
	useEffect(() => {
		...
		const uniforms = {
			...
			uColor3: {
				value: new THREE.Vector3(highlightColor.r, highlightColor.g, highlightColor.b),
			},
		...
		const updateMousePosition = () => {
			...
			uniforms.uMouseAlpha.value += (targetAlpha - currentAlpha) * alphaLerpFactor
		}
		...
		const renderLoop = (now: number) => {
			...
			if (isInView) {
				...
				if (timeSinceStart < delayMs) {
					...
					const progress = Math.min((timeSinceStart - delayMs) / 1000 / animationDurationSec, 1.0)
