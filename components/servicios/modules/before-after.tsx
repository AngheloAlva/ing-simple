...
function BeforeMock(): ReactNode {
  ...
  const cols = 7;
  return (
    <div className="flex h-full flex-col bg-muted/40 font-mono text-[10px] text-muted-foreground">
      {/* File chrome */}
      <div className="flex shrink-0 items-center justify-between border-b border-border/60 bg-background/70 px-3 py-2">
        <span className="truncate font-medium">
          control_operacional_v7_FINAL(3).xlsx
        </span>
      ...
      <div className="min-h-0 flex-1 overflow-hidden p-2">
        <div
          className="grid h-full gap-px bg-border/50"
        ...
        >
          {Array.from({ length: rows * cols }, (_, i) => {
            ...
            return (
              ...
              >
                {isHeader
                  ? ["Fecha", "OT", "Área", "Estado", "Resp.", "Monto", "Obs"][
                      i % cols
                    ]
                  : broken
                    ? "#¡REF!"
      ...
      {/* Sheet tabs */}
      <div className="flex shrink-0 items-center gap-1 border-t border-border/60 bg-background/70 px-2 py-1.5">
        {["Hoja1", "Copia de Hoja1", "FINAL", "no borrar"].map((tab, i) => (
          <span
            key={tab}
            className={`truncate px-2 py-0.5 ${
              i === 2 ? "bg-muted font-medium" : ""
            }`}
          >
            {tab}
          </span>
...
function AfterMock(): ReactNode {
  ...
  return (
    <div className="flex h-full bg-background text-[10px]">
      {/* Sidebar */}
      <aside className="hidden w-24 shrink-0 flex-col gap-1 border-r border-border/60 p-2 sm:flex">
        <span className="mb-2 h-4 w-4 rounded bg-primary" aria-hidden="true" />
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="mx-2.5 mb-2.5 flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-border/60">
          {rows.map((row) => (
            <div
              key={row.ot}
              className="flex flex-1 items-center justify-between border-b border-border/40 px-2.5 last:border-b-0"
            >
              <span className="font-medium text-foreground">{row.ot}</span>
    ...
    <section className="mx-auto max-w-[1440px] px-5 pb-24 sm:px-8 sm:pb-32 lg:px-10">
      <div className="mx-auto mb-12 max-w-2xl text-center sm:mb-16">
        ...
        <p className="mt-4 text-sm leading-relaxed text-muted-foreground sm:text-base">
          Arrastra el divisor: así se ve la misma operación antes y después de
      ...
      <div className="mx-auto max-w-[1100px]">
        <div
          ref={containerRef}
          className="relative h-[380px] touch-pan-y select-none overflow-hidden rounded-2xl border border-border/60 shadow-2xl shadow-black/[0.08] sm:h-[440px]"
          onPointerDown={(e) => {
        ...
        >
          ...
          {/* Layer labels */}
          <span className="pointer-events-none absolute left-3 top-3 z-10 rounded bg-foreground/80 px-2 py-1 text-[10px] font-semibold uppercase tracking-widest text-background">
            Antes
          </span>
          <span className="pointer-events-none absolute right-3 top-3 z-10 rounded bg-primary px-2 py-1 text-[10px] font-semibold uppercase tracking-widest text-primary-foreground">
            Después
          </span>
