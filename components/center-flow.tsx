...
const generatePathD = (from: { x: number; y: number }, to: { x: number; y: number }) => {
...
const CenterFlow: React.FC<CenterFlowProps> = ({
	...
	const nodeBgColor = isLight ? "rgba(255, 255, 255, 0.9)" : "rgba(10, 10, 10, 0.9)"
	...
	useEffect(() => {
		...
		const updateGlow = () => {
			if (glowRef.current) {
				...
				glowIntensityRef.current = dynamicIntensity > 0.5 ? dynamicIntensity * glowDecay : 0
			}
	...
	useEffect(() => {
		...
		const calculateSegments = () => {
			...
			for (const pulse of pulses) {
				...
				for (let i = 0; i <= 8; i++) {
					const point = path.getPointAtLength(length * (tailPos + (headPos - tailPos) * (i / 8)))
	...
	return (
		...
		>
			...
			<div
				ref={glowRef}
				className="absolute flex -translate-x-1/2 -translate-y-1/2 transform items-center justify-center transition-colors duration-300"
				style={centerStyle}
			>
				<div
