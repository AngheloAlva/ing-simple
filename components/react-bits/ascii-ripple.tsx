...
/** Resolves a CSS custom property before handing its concrete value to Canvas. */
export function resolveCanvasColor(
  input: string,
  getPropertyValue: (name: string) => string,
): string {
  const match = /^var\(\s*(--[\w-]+)\s*(?:,\s*([^)]*))?\)$/.exec(input);
  if (!match) return input;

  const resolved = getPropertyValue(match[1] ?? "").trim();
  return resolved || match[2]?.trim() || input;
}

export function canAnimateRipple({
  inViewport,
  documentVisible,
  reducedMotion,
}: {
  ...
  return inViewport && documentVisible && !reducedMotion;
}

export function canHandleRipplePointer(interactive: boolean, reducedMotion: boolean): boolean {
...
const parseColor = (input: string): Rgba => {
  ...
  return [d[0] ?? 255, d[1] ?? 255, d[2] ?? 255, (d[3] ?? 255) / 255];
};
  ...
  for (let y = 1; y < h - 1; y++) {
    ...
    for (let x = 1; x < w - 1; x++) {
      ...
      const lap =
        ...
        4 * c;
      const v =
        c * restore + (c - (prev[i] ?? 0)) * keep * (mask[i] ?? 1) + k * lap;
      next[i] = v;
      const a = v < 0 ? -v : v;
...
const disturb = (
  ...
  for (let y = y0; y <= y1; y++) {
    for (let x = x0; x <= x1; x++) {
      ...
      if (q <= 0) continue;
      const index = y * s.w + x;
        s.cur[index] = (s.cur[index] ?? 0) - amount * q * q * (3 - 2 * q);
    }
  }
...
const sampleSurface = (
  ...
  const { w, cur } = s;
  const i = y * w + x;
  const top = (cur[i] ?? 0) + ((cur[i + 1] ?? 0) - (cur[i] ?? 0)) * tx;
  const bottom =
        (cur[i + w] ?? 0) + ((cur[i + w + 1] ?? 0) - (cur[i + w] ?? 0)) * tx;
  out[0] = top + (bottom - top) * ty;
  const gxTop = (cur[i + 1] ?? 0) - (cur[i - 1] ?? 0);
  const gxBottom = (cur[i + w + 1] ?? 0) - (cur[i + w - 1] ?? 0);
  out[1] = (gxTop + (gxBottom - gxTop) * ty) * 0.5;
  const gyLeft = (cur[i + w] ?? 0) - (cur[i - w] ?? 0);
  const gyRight = (cur[i + w + 1] ?? 0) - (cur[i - w + 1] ?? 0);
...
const AsciiRipple = forwardRef<AsciiRippleHandle, AsciiRippleProps>(
  (
    ...
    const ramps = useMemo(() => {
      const base = parseColor(resolvedColors.text);
      const crest = parseColor(resolvedColors.ripple);
      const trough = parseColor(resolvedColors.trough);
      const up: string[] = [];
      const down: string[] = [];
      ...
      return { up, down };
    }, [resolvedColors.ripple, resolvedColors.text, resolvedColors.trough]);
    const live = {
      chars,
      font,
      speed,
    ...
    const resolveColors = useCallback(() => {
        ...
        const next = {
          text: resolveCanvasColor(textColor, styles.getPropertyValue.bind(styles)),
          ripple: resolveCanvasColor(rippleColor, styles.getPropertyValue.bind(styles)),
          trough: resolveCanvasColor(troughColor, styles.getPropertyValue.bind(styles)),
          background: resolveCanvasColor(backgroundColor, styles.getPropertyValue.bind(styles)),
        };
        setResolvedColors((current) =>
          current.text === next.text &&
          current.ripple === next.ripple &&
          current.trough === next.trough &&
          current.background === next.background
            ? current
            : next,
        );
      }, [backgroundColor, rippleColor, textColor, troughColor]);
      useEffect(() => {
        resolveColors();
        const observer = new MutationObserver(resolveColors);
            const serviceScope = rootRef.current?.closest("[data-service]");
            if (serviceScope) {
              observer.observe(serviceScope, {
                attributes: true,
                attributeFilter: ["data-service", "style"],
              });
            }
      ...
      ctx.fillStyle = p.ramps.up[0] ?? "rgba(255,255,255,1)";
      for (let r = 0; r < rows; r++) {
        const row = base[r] ?? [];
        ...
        if (!active) {
          ...
          for (let c = 0; c < cols; c++) {
            ...
            const hgt = out[0] ?? 0;
            const gx = (out[1] ?? 0) * slopeScale;
            const gy = (out[2] ?? 0) * slopeScale;
            ...
            hot.push({
              ...
              y: cy,
              ch: glyph,
              color: ramp[Math.round(intensity * RAMP_STEPS)] ?? ramp[0] ?? "rgba(255,255,255,1)",
            });
            line += " ";
          }
        }
    ...
    useEffect(() => {
      const visibilityRoot = rootRef.current;
        ...
        const visibilityObserver = new IntersectionObserver(([entry]) => {
          ...
          if (inViewportRef.current) wake();
          else stopLoop();
        });
        updateDocumentVisibility();
        if (visibilityRoot) visibilityObserver.observe(visibilityRoot);
    ...
    }, [rebuild, stopLoop, wake]);
    useEffect(() => {
      draw(performance.now());
    }, [ramps, chars, textOpacity, vignette, resolvedColors.background, draw]);
    ...
    const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
          if (!canHandleRipplePointer(interactive, reducedRef.current)) return;
